...
public class Block {
	...
	public static int benchmark(int upperMillis) {
		...
		try {
			System.out.println("*** Benchmark - looking for difficulty with upper bound of " + upperMillis + "ms");
			outer:for (; tmpDiff < 10; ++tmpDiff) {
				System.out.print("Difficulty=" + tmpDiff + " ");
				for (int j = 0; j < 100; ++j) {
					b = new Block("test");
			...
			System.out.println("*** Difficulty=" + tmpDiff);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return tmpDiff;
